...
interface DisbursementDetailSheetProps {
  ...
  onReject: () => void; // It will just open the dialog
}
...
export function DisbursementDetailSheet({ isOpen, onClose, credit, onDisburse, onReject }: DisbursementDetailSheetProps) {
  ...
  const handleAction = async (action: 'disburse' | 'reject') => {
      setIsProcessing(true);
      if (action === 'disburse') {
          await onDisburse();
      } else {
          onReject(); // This will just trigger the dialog opening
      }
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-full max-w-md flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 py-4 pr-2">
            ...
            <DetailRow label="Fecha Primera Cuota" value={formatDate(credit.firstPaymentDate)} />
        </div>
        ...
        <SheetFooter className="mt-auto pt-4 border-t">
          {credit.status === 'Approved' ? (
            <div className="grid grid-cols-2 gap-2 w-full">
              ...
              <Button size="lg" onClick={() => handleAction('disburse')} disabled={isProcessing} className="bg-accent text-accent-foreground hover:bg-accent/90">
